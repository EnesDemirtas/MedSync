...
func (s *Store) ExecuteUnderTransaction(tx transaction.Transaction) (tag.Storer, error) {
...
func (s *Store) Create(ctx context.Context, tag tag.Tag) error {
	const q = `
	INSERT INTO tags
		(tag_id, name)
	VALUES
...
func (s *Store) Delete(ctx context.Context, tag tag.Tag) error {
	data := struct {
		ID string `db:"tag_id"`
	}{
		ID: tag.ID.String(),
...
func (s *Store) Update(ctx context.Context, tag tag.Tag) error {
	const q = `
	UPDATE
		tags
	SET
...
func (s *Store) Query(ctx context.Context, filter tag.QueryFilter, orderBy order.By, pageNumber int, rowsPerPage int) ([]tag.Tag, error) {
...
func (s *Store) Count(ctx context.Context, filter tag.QueryFilter) (int, error) {
	data := map[string]interface{}{}
...
func (s *Store) QueryByID(ctx context.Context, tagID uuid.UUID) (tag.Tag, error) {
	data := struct {
		ID string `db:"tag_id"`
	}{
		ID: tagID.String(),
	...
	if err := sqldb.NamedQueryStruct(ctx, s.log, s.db, q, data, &dbTag); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return tag.Tag{}, fmt.Errorf("db: %w", tag.ErrNotFound)
		}
		return tag.Tag{}, fmt.Errorf("db: %w", err)
	}
...
func (s *Store) QueryByIDs(ctx context.Context, tagIDs []uuid.UUID) ([]tag.Tag, error) {
