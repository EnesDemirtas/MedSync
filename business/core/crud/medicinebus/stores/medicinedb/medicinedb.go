...
func (s *Store) ExecuteUnderTransaction(tx transaction.Transaction) (medicinebus.Storer, error) {
...
func (s *Store) Create(ctx context.Context, med medicinebus.Medicine) error {
	const q = `
	...
	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, toDBMedicine(med)); err != nil {
		if errors.Is(err, sqldb.ErrDBDuplicatedEntry) {
			return fmt.Errorf("namedexeccontext: %w", medicinebus.ErrUniquePK)
		}
...
func (s *Store) Update(ctx context.Context, med medicinebus.Medicine) error {
	const q = `
	UPDATE
		medicines
	SET
	...
	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, toDBMedicine(med)); err != nil {
		if errors.Is(err, sqldb.ErrDBDuplicatedEntry) {
			return medicinebus.ErrUniquePK
		}
		return fmt.Errorf("namedexeccontext: %w", err)
	}
...
func (s *Store) Delete(ctx context.Context, med medicinebus.Medicine) error {
	data := struct {
...
func (s *Store) Query(ctx context.Context, filter medicinebus.QueryFilter, orderBy order.By, pageNumber int, rowsPerPage int) ([]medicinebus.Medicine, error) {
...
func (s *Store) Count(ctx context.Context, filter medicinebus.QueryFilter) (int, error) {
	data := map[string]interface{}{}
...
func (s *Store) QueryByID(ctx context.Context, medicineID uuid.UUID) (medicinebus.Medicine, error) {
	data := struct {
		ID string `db:"medicine_id"`
	}{
	...
	if err := sqldb.NamedQueryStruct(ctx, s.log, s.db, q, data, &dbMedicine); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return medicinebus.Medicine{}, fmt.Errorf("db: %w", medicinebus.ErrNotFound)
		}
		return medicinebus.Medicine{}, fmt.Errorf("db: %w", err)
	}
...
func (s *Store) QueryByIDs(ctx context.Context, medicineIDs []uuid.UUID) ([]medicinebus.Medicine, error) {
	...
	if err := sqldb.NamedQuerySlice(ctx, s.log, s.db, q, data, &dbMedicines); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return nil, medicinebus.ErrNotFound
		}
...
func (s *Store) QueryByName(ctx context.Context, name string) (medicinebus.Medicine, error) {
	data := struct {
		Name string `db:"name"`
	}{
		Name: name,
