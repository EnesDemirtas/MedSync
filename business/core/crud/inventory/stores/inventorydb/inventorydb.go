...
func (s *Store) ExecuteUnderTransaction(tx transaction.Transaction) (inventory.Storer, error) {
...
func (s *Store) Create(ctx context.Context, inv inventory.Inventory) error {
	const q = `
	...
	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, toDBInventory(inv)); err != nil {
		if errors.Is(err, sqldb.ErrDBDuplicatedEntry) {
			return fmt.Errorf("namedexeccontext: %w", inventory.ErrUniquePK)
		}
...
func (s *Store) Update(ctx context.Context, inv inventory.Inventory) error {
	const q = `
	UPDATE
		inventories
	SET
...
func (s *Store) Delete(ctx context.Context, inv inventory.Inventory) error {
	data := struct {
...
func (s *Store) Query(ctx context.Context, filter inventory.QueryFilter, orderBy order.By, pageNumber int, rowsPerPage int) ([]inventory.Inventory, error) {
...
func (s *Store) Count(ctx context.Context, filter inventory.QueryFilter) (int, error) {
	data := map[string]interface{}{}
...
func (s *Store) QueryByID(ctx context.Context, inventoryID uuid.UUID) (inventory.Inventory, error) {
	data := struct {
		ID string `db:"inventory_id"`
	}{
	...
	if err := sqldb.NamedQueryStruct(ctx, s.log, s.db, q, data, &dbInventory); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return inventory.Inventory{}, fmt.Errorf("db: %w", inventory.ErrNotFound)
		}
		return inventory.Inventory{}, fmt.Errorf("db: %w", err)
	}
...
func (s *Store) QueryByIDs(ctx context.Context, inventoryIDs []uuid.UUID) ([]inventory.Inventory, error) {
...
func (s *Store) QueryByName(ctx context.Context, name string) (inventory.Inventory, error) {
	data := struct {
		Name string `db:"name"`
	}{
		Name: name,
