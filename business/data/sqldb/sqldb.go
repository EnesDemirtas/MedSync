...
// Set of error variable for CRUD operations.
var (
...
// ExecContext is a helper function to execute a CRUD operation with
// logging and tracing.
...
// single value to be unmarshalled into a struct type where field replacement is neccessary.
...
func namedQueryStruct(ctx context.Context, log *logger.Logger, db sqlx.ExtContext, query string, data any, dest any, withIn bool) (err error) {
	...
	defer func() {
		if err != nil {
			log.Infoc(ctx, 6, "database.NamedQueryStruct", "query", q, "ERROR", err)
		}
	}()
