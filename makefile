...
build: warehouse metrics auth

warehouse:
	docker build \
		-f zarf/docker/dockerfile.warehouse \
		-t $(WAREHOUSE_IMAGE) \
...
dev-load:
	kind load docker-image $(WAREHOUSE_IMAGE) --name $(KIND_CLUSTER)
...
dev-apply:
	...
	kustomize build zarf/k8s/dev/warehouse | kubectl apply -f -
	kubectl wait pods --namespace=$(NAMESPACE) --selector app=$(WAREHOUSE_APP) --timeout=120s --for=condition=Ready

dev-restart:
	kubectl rollout restart deployment $(WAREHOUSE_APP) --namespace=$(NAMESPACE)
...
dev-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | go run apis/tooling/logfmt/main.go -service=$(WAREHOUSE_APP)
...
dev-logs-init:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP) -f --tail=100 -c init-migrate-seed

dev-describe-node:
	kubectl describe node

dev-describe-deployment:
	kubectl describe deployment --namespace=$(NAMESPACE) $(WAREHOUSE_APP)

dev-describe-warehouse:
	kubectl describe pod --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP)
...
dev-services-delete:
	kustomize build zarf/k8s/dev/warehouse | kubectl delete -f -
...
dev-describe-replicaset:
	kubectl get rs
	kubectl describe rs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP)
...
dev-shell:
	kubectl exec --namespace=$(NAMESPACE) -it $(shell kubectl get pods --namespace=$(NAMESPACE) | grep warehouse | cut -c1-26) --container warehouse-api -- /bin/sh
...
migrate:
	export WAREHOUSE_DB_HOST_PORT=localhost; go run apis/tooling/admin/main.go migrate

seed: migrate
	export WAREHOUSE_DB_HOST_PORT=localhost; go run apis/tooling/admin/main.go seed
...
token-gen:
	export WAREHOUSE_DB_HOST_PORT=localhost; go run apis/tooling/admin/main.go gentoken 5cf37266-3473-4006-984f-9325122678b7 54bb2165-71e1-41a6-af3e-7da4a0e1e2c1
...
run:
	go run apis/services/warehouse/main.go | go run apis/tooling/logfmt/main.go

run-help:
	go run apis/services/warehouse/main.go --help | go run apis/tooling/logfmt/main.go
...
talk-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP) --all-containers=true -f --tail=100 --max-log-requests=6

talk-logs-cpu:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep SCHED

talk-logs-mem:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(WAREHOUSE_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep "ms clock"
